<template>
  ...
  >
    <el-form :model="form" label-width="120">
      ...
      /></el-form-item>
      <el-form-item label="促銷價"
        ><el-input-number v-model="form.promo_price" :min="0" :step="10"
      /></el-form-item>
      ...
      <template v-else>
        <UploadGallery
          ...
          :upload-url="`/api/upload/products/${form.id}`"
          title="商品圖片"
        />
      </template>
    </el-form>
...
async function save() {
  ...
  if (!form.id) {
    ...
    if (form.images_urls?.length)
      await api.post(`/products/${form.id}/images`, form.images_urls);
  }
  show.value = false;
  fetchAll();
}
