<template>
  ...
  <el-drawer v-model="show" size="60%" :title="form.id ? '編輯布料' : '新增布料'">
    <el-form :model="form" label-width="120">
      ...
      <el-form-item label="尺寸"><el-input v-model="form.size" placeholder="例如：110x90cm"/></el-form-item>
      <el-form-item label="描述"><el-input type="textarea" v-model="form.description"/></el-form-item>
      <el-form-item label="是否出清"><el-switch v-model="form.on_clearance"/></el-form-item>
      <el-form-item label="出清價格">
        <el-input-number v-model="form.clearance_price" :min="0" :step="10" :disabled="!form.on_clearance"/>
      </el-form-item>
      ...
      <template v-else>
        ...
        <UploadGallery v-model="form.works_urls" :upload-url="`/api/upload/fabrics/${form.id}?kind=work`" title="作品圖片"/>
      </template>
    </el-form>
...
function openForm(row) {
  show.value = true
  Object.assign(form, {
    id: null, name: '', origin: '台灣', price: 0, size: '',
    description: '', on_clearance: false, clearance_price: 0,
    images_urls: [], works_urls: []
  })
...
function onPickPending(e, kind) {
  ...
  if (kind === 'image') {
    ...
    pendingImagesPreviews.value.push(...files.map(f => URL.createObjectURL(f)))
  } else {
    pendingWorksFiles.value.push(...files)
    pendingWorksPreviews.value.push(...files.map(f => URL.createObjectURL(f)))
  }
}

async function save() {
  const payload = {
    name: form.name, origin: form.origin, price: form.price,
    size: form.size, description: form.description,
    on_clearance: form.on_clearance, clearance_price: form.clearance_price
  }
  ...
  if (!form.id) {
    ...
    if (pendingImagesFiles.value.length) {
      const r = await fileUpload(`/api/upload/fabrics/${id}?kind=image`, pendingImagesFiles.value)
      const urls = r.data.saved || []
      if (urls.length) await api.post(`/fabrics/${id}/images`, urls)
    }
    if (pendingWorksFiles.value.length) {
      const r = await fileUpload(`/api/upload/fabrics/${id}?kind=work`, pendingWorksFiles.value)
      const urls = r.data.saved || []
      if (urls.length) await api.post(`/fabrics/${id}/works`, urls)
    }
  } else {
    await api.put(`/fabrics/${form.id}`, payload)
    if (form.images_urls?.length) await api.post(`/fabrics/${form.id}/images`, form.images_urls)
    if (form.works_urls?.length) await api.post(`/fabrics/${form.id}/works`, form.works_urls)
  }
